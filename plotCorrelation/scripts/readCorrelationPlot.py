...
parser = argparse.ArgumentParser(description="Calculates the correlation \
    of each samples within a counts matrix (samples x features) \
    and then performs to agglomerative (bottom-up) hierarchical clustering \
...
parser.add_argument('-cm','--corr_method', help='method used \
    to calculate pairwise correlation (default= "pearson")', 
    choices=['pearson', 'kendall', 'spearman'], default="pearson")
...
parser.add_argument('-k','--k_clusters', 
    help='number of clusters to color (default:2)', 
    type=int, default=2)
parser.add_argument('-ri','--calcRandIndexForSpecificClustering', 
    action="store_true",
    help = 'Set this if you want to calculate and print the rand-index(cies) \
    between user-predicted clusters (set using the `--cluster_file` parameter) \
    and the unsupervised clustering.')
parser.add_argument('-cf','--cluster_file',type=str,
    help='If `calcRandIndexForSpecificClustering` is set, this \
    parameter must be set to a path to a file containing the users \
    predicted clusters. \
    The file must be formatted as a comma-delimited table. \
    The first column must be labeled "samples". The rest of the column \
    names will represents the clustering types. The "samples" column \
    includes the sample IDs found in the header of `infile`. \
    The rest of the columns should have labels starting from 1 \
    for the clusters you want to compare.')
parser.add_argument('-sl','--samplabels',
    help = 'If set, this parameter must be set to a path of a file \
    containing meta data for plotting the samples.The file must \
    be formatted as a comma-delimited table. \
    The first column labeled "samples" \
    should include the sample IDs found in the header of `infile`. \
    The rest of the column(s) should include both or either the `color` \
    and `shape` column. The colors can be given in any format recognized \
    by matplotlib. See https://matplotlib.org/3.1.1/api/markers_api.html \
...
for k in range(2,n):
    ...
    colname="0_"+str(k)
    if k==2:
        cluster_df = pd.DataFrame({"samples":sampList,
            colname:fclust})
    else:
        cluster_df[colname]=fclust
cluster_df.set_index('samples', inplace=True)
...
if args.samplabels:
    ...
    samplabels_df["s_idx"] = samplabels_df["samples"].apply(lambda x:denOrder_dic[x])
    samplabels_df = samplabels_df.set_index("s_idx")
    samplabels_df = samplabels_df.sort_index()
    samplabels_df = samplabels_df.set_index("samples")
# plot heatmap
