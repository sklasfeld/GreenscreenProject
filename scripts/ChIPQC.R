...
runchipqc <- function(exp_obj, out_path, facet=NULL, colour=NULL){
  if ((!is.null(facet)) &&
    ...
    #update_geom_defaults("line", list(size = 3, alpha=.5))
    #q <- plotCC(exp_obj,facetBy=facet,
    #  colourBy=colour) +
    #  scale_color_manual(values=c('black','darkgreen','blue')) 
    #ggsave(plot = q, width = 6, height = 3, 
...
parser$add_argument("-i", "--indivReports", action="store_true", default=F,
                    help="Print a seperate report for each sample")
