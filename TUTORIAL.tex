...
The raw sequences of the twenty inputs in Table S1 are retrieved using NCBI SRA Tools ({\fontfamily{cmtt}\selectfont scripts/import\_raw\_fasta\_inputs.sh}). To organize these fastq files, the files are compressed and moved into the `fastq/raw` directory and renamed based on the sample ID given to then in Table S1 ({\fontfamily{cmtt}\selectfont scripts/organize\_raw\_fasta\_input.sh}). Note that InputJ contains three run files; these reads are concatenated and the individual run files are deleted.
